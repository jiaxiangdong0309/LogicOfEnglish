...
import type { Phonogram } from '@/lib/phonogramData';

interface PhonogramCardProps {
  phonogram: Phonogram;
}

export default function PhonogramCard({ phonogram }: PhonogramCardProps) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className={`font-semibold ${textColor}`}>
              {phonogram.sounds.length} sound{phonogram.sounds.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        <ChevronDown
